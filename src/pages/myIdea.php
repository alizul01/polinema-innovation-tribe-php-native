...
if (isset($_GET['inputSearch'])) {
    ...
    $query = "SELECT user.ID, user.NAMA, user.FOTO, idea_box.CREATED_AT, idea_box.THUMBNAIL, idea_box.DESKRIPSI_SINGKAT, idea_box.ID_IDEA, idea_box.KATEGORI, idea_box.JUDUL, idea_box.DESKRIPSI FROM user INNER JOIN idea_box ON user.id = idea_box.id WHERE user.id = " . $_SESSION['id'] . " AND idea_box.JUDUL LIKE '%$search%'";
    $result = mysqli_query($conn, $query);
    $ideas = mysqli_fetch_all($result, MYSQLI_ASSOC);
}
            ...
            <div class="col-12">
                <div class="row justify-content-between">
                    ...
                    <form class="input-group" action="#" method="GET">
                            <input type="text" name="inputSearch" id="inputSearch" class="form-control" placeholder="Search" aria-label="Search" aria-describedby="button-addon2" value="<?= $_GET['inputSearch'] ?? '' ?>">
